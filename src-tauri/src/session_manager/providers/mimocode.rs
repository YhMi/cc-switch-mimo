...
/// Return the MimoCode base directory (`$XDG_DATA_HOME/mimocode`).
///
...
pub(crate) fn get_mimocode_base_dir() -> PathBuf {
    if let Ok(xdg) = std::env::var("XDG_DATA_HOME") {
        if !xdg.is_empty() {
            return PathBuf::from(xdg).join("mimocode");
        }
    }
    dirs::home_dir()
        .map(|h| h.join(".local/share/mimocode"))
        .unwrap_or_else(|| PathBuf::from(".local/share/mimocode"))
}

/// Return the MimoCode JSON storage directory (legacy flat-file layout).
pub(crate) fn get_mimocode_data_dir() -> PathBuf {
    get_mimocode_base_dir().join("storage")
}

fn get_mimocode_db_path() -> PathBuf {
    get_mimocode_base_dir().join("mimocode.db")
}
...
fn scan_sessions_json() -> Vec<SessionMeta> {
    let storage = get_mimocode_data_dir();
...
/// This relies on the MimoCode convention that session IDs start with `ses_`.
...
fn scan_sessions_sqlite() -> Vec<SessionMeta> {
    ...
    for row in iter.flatten() {
        ...
        sessions.push(SessionMeta {
            ...
            resume_command: Some(format!("mimocode session resume {session_id}")),
        });
    }
    sessions
}
...
/// Load messages from the MimoCode SQLite database for a given source reference.
...
pub fn delete_session(storage: &Path, path: &Path, session_id: &str) -> Result<bool, String> {
    if path.file_name().and_then(|name| name.to_str()) != Some(session_id) {
        return Err(format!(
            "MimoCode session path does not match session ID: expected {session_id}, found {}",
            path.display()
        ));
    }
    ...
    for message_id in &message_ids {
        ...
        remove_dir_all_if_exists(&part_dir).map_err(|e| {
            format!(
                "Failed to delete MimoCode part directory {}: {e}",
                part_dir.display()
            )
        })?;
    }
    ...
    remove_file_if_exists(&session_diff_path).map_err(|e| {
        format!(
            "Failed to delete MimoCode session diff {}: {e}",
    ...
    remove_dir_all_if_exists(path).map_err(|e| {
        format!(
            "Failed to delete MimoCode message directory {}: {e}",
            path.display()
        )
    })?;
    ...
    if let Some(session_file) = find_session_file(storage, session_id) {
        remove_file_if_exists(&session_file).map_err(|e| {
            format!(
                "Failed to delete MimoCode session file {}: {e}",
...
/// Delete a session from the MimoCode SQLite database.
pub fn delete_session_sqlite(session_id: &str, source: &str) -> Result<bool, String> {
    ...
    let expected_db_path = get_mimocode_db_path()
        .canonicalize()
        .map_err(|e| format!("Failed to canonicalize expected MimoCode database path: {e}"))?;

    if ref_session_id != session_id {
        return Err(format!(
            "MimoCode SQLite session ID mismatch: expected {session_id}, found {ref_session_id}"
        ));
    }
    if db_path != expected_db_path {
        return Err("SQLite path does not match expected MimoCode database".to_string());
    }

    let conn =
        Connection::open(&db_path).map_err(|e| format!("Failed to open MimoCode database: {e}"))?;
    ...
    tx.execute("DELETE FROM part WHERE session_id = ?1", [session_id])
        .map_err(|e| format!("Failed to delete MimoCode parts: {e}"))?;
    tx.execute("DELETE FROM message WHERE session_id = ?1", [session_id])
        .map_err(|e| format!("Failed to delete MimoCode messages: {e}"))?;

    let deleted = tx
        .execute("DELETE FROM session WHERE id = ?1", [session_id])
        .map_err(|e| format!("Failed to delete MimoCode session: {e}"))?;
...
mod tests {
    ...
    fn mimocode_env_lock() -> &'static Mutex<()> {
    ...
    fn parse_sqlite_source_accepts_valid_references() {
        let parsed = parse_sqlite_source("sqlite:/tmp/mimocode.db:ses_123").expect("valid source");

        assert_eq!(parsed.0, PathBuf::from("/tmp/mimocode.db"));
        assert_eq!(parsed.1, "ses_123");
    }

    #[test]
    fn parse_sqlite_source_rejects_invalid_references() {
        assert!(parse_sqlite_source("/tmp/mimocode.db:ses_123").is_none());
        assert!(parse_sqlite_source("sqlite:/tmp/mimocode.db:msg_123").is_none());
        assert!(parse_sqlite_source("sqlite:/tmp/mimocode.db").is_none());
    }
    ...
    fn scan_sessions_sqlite_reads_temp_database() {
        let _guard = mimocode_env_lock().lock().expect("lock");
        let temp = tempdir().expect("tempdir");
        let original_xdg = std::env::var_os("XDG_DATA_HOME");
        std::env::set_var("XDG_DATA_HOME", temp.path());

        let base_dir = temp.path().join("mimocode");
    ...
    fn load_messages_sqlite_reads_messages_and_parts() {
        let temp = tempdir().expect("tempdir");
        let db_path = temp.path().join("mimocode.db");
        let conn = Connection::open(&db_path).expect("open sqlite db");
        create_sqlite_schema(&conn);
    ...
    fn delete_session_sqlite_rejects_foreign_db_path() {
        ...
        let expected_base_dir = temp.path().join("mimocode");
        std::fs::create_dir_all(&expected_base_dir).expect("create expected base dir");
        let expected_db_path = expected_base_dir.join("mimocode.db");
        ...
        assert!(err.contains("expected MimoCode database"));
