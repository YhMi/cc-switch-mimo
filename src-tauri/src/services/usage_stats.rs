...
pub(crate) fn has_matching_proxy_usage_log(
    ...
    let allow_missing_cache_creation =
        matches!(key.app_type, "codex" | "gemini" | "opencode"|"mimocode") && key.cache_creation_tokens == 0;
...
mod tests {
    ...
    #[test]
    fn test_get_provider_stats_labels_mimocode_session_provider() -> Result<(), AppError> {
        let db = Database::memory()?;
        ...
        let stats = db.get_provider_stats(None, None, Some("mimocode"), None, None)?;
        assert_eq!(stats.len(), 1);
        assert_eq!(stats[0].provider_id, "_mimocode_session");
        assert_eq!(stats[0].provider_name, "MimoCode (Session)");
