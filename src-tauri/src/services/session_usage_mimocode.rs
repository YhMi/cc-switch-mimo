//! MimoCode 会话日志使用追踪
//!
//! 从 ~/.local/share/mimocode/mimocode.db (SQLite) 中提取精确 token 使用数据。
//!
//! ## 数据流
//! ```text
...
use crate::mimocode_config::get_mimocode_db_path;
...
/// 从 mimocode message.data JSON 中提取的 token 和费用数据
...
struct MimoCodeMessageQueryResult {
    messages: Vec<(String, MimoCodeMessageData)>,
...
pub fn sync_mimocode_usage(db: &Database) -> Result<SessionSyncResult, AppError> {
    ...
    // 检查文件修改时间。
    // mimocode 的数据库运行在 WAL 模式：新提交先落在 -wal 文件里，
    // 主库文件只有在 checkpoint 时才更新。因此必须同时考虑 -wal 的
    // mtime，否则会在 checkpoint 之前漏掉刚写入的会话。
    let metadata = fs::metadata(&db_path)
        .map_err(|e| AppError::Config(format!("无法读取 mimocode.db 元数据: {e}")))?;
    ...
    // 打开 mimocode 的 SQLite 数据库（只读）
    let mimocode_conn =
        rusqlite::Connection::open_with_flags(&db_path, rusqlite::OpenFlags::SQLITE_OPEN_READ_ONLY)
            .map_err(|e| AppError::Database(format!("无法打开 mimocode.db: {e}")))?;
    ...
    // 查询所有会话
    let sessions = query_sessions(&mimocode_conn)?;
    ...
    for (session_id, time_updated) in &sessions {
        ...
        match query_assistant_messages(&mimocode_conn, session_id) {
            Ok(query_result) => {
                session_has_incomplete_usage = query_result.has_incomplete_usage;
                for (message_id, msg_data) in &query_result.messages {
                    let request_id = format!("mimocode_session:{session_id}:{message_id}");

                    match insert_mimocode_message(db, &request_id, msg_data, session_id) {
                        Ok(true) => result.imported += 1,
                        Ok(false) => result.skipped += 1,
                        Err(e) => {
                            let msg = format!("MimoCode 消息插入失败 {request_id}: {e}");
                            log::warn!("[MIMOCODE-SYNC] {msg}");
            ...
            Err(e) => {
                let msg = format!("MimoCode 会话消息查询失败 {session_id}: {e}");
                log::warn!("[MIMOCODE-SYNC] {msg}");
                result.errors.push(msg);
                session_had_error = true;
            }
        ...
        if let Err(e) = update_sync_state(db, &sync_key, *time_updated, 0) {
            let msg = format!("MimoCode 会话同步状态更新失败 {session_id}: {e}");
            log::warn!("[MIMOCODE-SYNC] {msg}");
            result.errors.push(msg);
            has_sync_errors = true;
        }
    ...
    if result.imported > 0 {
        log::info!(
            "[MIMOCODE-SYNC] 同步完成: 导入 {} 条, 跳过 {} 条, 扫描 {} 个会话",
            result.imported,
            result.skipped,
            sessions.len()
        );
...
fn query_assistant_messages(
    ...
    session_id: &str,
) -> Result<MimoCodeMessageQueryResult, AppError> {
    let mut stmt = conn
    ...
    Ok(MimoCodeMessageQueryResult {
        messages,
        has_incomplete_usage,
    })
}

/// 解析 mimocode message.data JSON 为结构化数据
...
/// 插入单条 MimoCode 消息记录到 proxy_request_logs
fn insert_mimocode_message(
    db: &Database,
    request_id: &str,
    msg: &MimoCodeMessageData,
    session_id: &str,
    ...
    // MimoCode 使用 Anthropic 风格：input 是新鲜输入，cache 单独计
    ...
    // 如果 mimocode 已经提供了费用，直接使用；否则从模型定价计算
    let (input_cost, output_cost, cache_read_cost, cache_creation_cost, total_cost) =
        if msg.cost > 0.0 {
            // mimocode 已计算费用，直接使用
            // 简化处理：全部放入 total_cost（mimocode 的 cost 是聚合值，无法精确拆分）
            (
                "0".to_string(),
                "0".to_string(),
                "0".to_string(),
            ...
            // mimocode 费用为 0（如免费模型），尝试用 cc-switch 自带的模型定价计算
    ...
    let inserted_rows = conn.execute(
        ...
        rusqlite::params![
            ...
            Some("mimocode_session"), // provider_type
            1i64,                  // is_streaming
            "1.0",                 // cost_multiplier
            created_at,
            "mimocode_session",    // data_source
        ],
    )
