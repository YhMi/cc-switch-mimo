...
> = {
  ...
  mimocode: { key: "mimocode", defaultFolder: ".config/mimocode" },
    ...
    const load = async () => {
      try {
        const [
          ...
          settingsApi.getConfigDir("mimocode"),
          ...
          computeDefaultConfigDir("mimocode"),
        ...
        setResolvedDirs({
          ...
          mimocode: mimoCodeDir || defaultsRef.current.mimocode,
  ...
  const resetAllDirectories = useCallback(
    (overrides?: ResolvedAppDirectoryOverrides) => {
      ...
      setResolvedDirs({
        ...
        mimocode: overrides?.mimocode ?? defaultsRef.current.mimocode,
