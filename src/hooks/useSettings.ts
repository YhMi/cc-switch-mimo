...
export function useSettings(): UseSettingsResult {
  ...
  const resetSettings = useCallback(() => {
    ...
    resetAllDirectories({
      ...
      mimocode: sanitizeDir(data?.mimocodeConfigDir),
    ...
    async (updates: Partial<SettingsFormState>): Promise<SaveResult | null> => {
      ...
      try {
        ...
        const payload: Settings = {
          ...
          mimocodeConfigDir: sanitizedMimocodeDir,
  ...
  const saveSettings = useCallback(
    async (
      ...
      try {
        ...
        const previousMimocodeDir = sanitizeDir(data?.mimocodeConfigDir);
        ...
        // 如果 Claude/Codex/Gemini/OpenCode/MimoCode/OpenClaw 的目录覆盖发生变化，则立即将"当前使用的供应商"写回对应应用的 live 配置
        // 如果插件同步已经执行过 syncCurrentProvidersLiveSafe，则跳过避免重复
        const claudeDirChanged = sanitizedClaudeDir !== previousClaudeDir;
        const codexDirChanged = sanitizedCodexDir !== previousCodexDir;
        const geminiDirChanged = sanitizedGeminiDir !== previousGeminiDir;
        const opencodeDirChanged = sanitizedOpencodeDir !== previousOpencodeDir;
        const mimoCodeDirChanged = sanitizedMimocodeDir !== previousMimocodeDir;
